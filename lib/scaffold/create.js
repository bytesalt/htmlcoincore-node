...
var BASE_PACKAGE = {
  description: 'HTMLCOIN',
  repository: 'https://github.com/bytesalt/htmlcoincore-node',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    ...
    'htmlcoincore-node': 'git://github.com/bytesalt/htmlcoincore-node.git'
  }
};
...
function createBitcoinDirectory(datadir, done) {
  fs.mkdirSync(datadir, function(err) {
    if (err) {
      throw err;
    }
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  fs.mkdir(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/htmlcoincore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the htmlcoincore-node directory and configuration
