...
HTMLCOIN.prototype.subscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'subscribe:', 'htmlcoind/addressbalance', 'total:', _.size(this.subscriptions.balance));
};
...
HTMLCOIN.prototype.unsubscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'htmlcoind/addressbalance', 'total:', _.size(this.subscriptions.balance));
};
    ...
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('htmlcoind/addressbalance', {
                    address: address,
                    txid: txid,
